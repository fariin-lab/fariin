    ...
    var body: some View {
        NavigationStack(path: $path) {
            ...
            .fullScreenCover(item: $viewerGroup) { g in
                ...
                // The zoom transition can install the SYSTEM's own interactive drag-down dismiss (shrinks
                // the whole viewer into a floating rounded card exposing the chat list) — a second close
                // style racing the library's linear slide pan. Kill it: swipes belong to the library pan
                // only. The hero zoom open/close ANIMATION is unaffected; programmatic close still works.
