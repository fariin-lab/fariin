...
// DEBUG ONLY — never compiled into Release (TestFlight / App Store).
//
...
// the url — so the viewer takes its ordinary "already downloaded" path and never learns these clips
...
//
// ⚠️ THE CLIPS CARRY A RUNNING CLOCK ON PURPOSE. Every frame prints its own timestamp and draws a
...
enum DemoStoryMedia {
    ...
    /// A round portrait. Avatars are read from `DiskImageCache`, NOT `URLCache` — see the note on
    /// `DemoMode.profilePhoto`, which found this the hard way — so it goes in both.
    ...
    /// ⚠️ MIRRORS `CacheManager.cacheFileName` AND MUST KEEP MIRRORING IT. That type is internal to
    /// StoryUI, so this cannot call it. The rule there is `lastPathComponent` with slashes flattened;
    /// every url built above is a plain single-component name, so both derivations agree trivially —
    ...
    /// Write a real H.264 mp4. Runs synchronously on whatever thread calls it — which is a background
