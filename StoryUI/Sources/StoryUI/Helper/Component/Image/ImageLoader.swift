...
final class ImageLoader: UIView {
    ...
    // Baked backdrops cached by URL — stable across re-applies and viewer rebuilds (upload handoff).
    ...
    // Downscale → gaussian σ16 → slight desaturation → ~50% near-black veil: material-matched
    // replacement for the old systemThickMaterialDark. Keep in sync with the host's StoryBlurBake.
...
private extension ImageLoader {
   func setupImageView() {
       ...
       // NO live UIVisualEffectView over the fill: live blur breaks while its opacity animates —
       // the story layer fades during the viewers-sheet crossfade and the blur dropped out,
       // flashing the raw bright fill ("reflection flashing"). The backdrop is a PRE-BAKED
       // material-matched blur set in apply(): pixel-stable through any fade.
