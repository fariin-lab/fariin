            ...
            .overlay(
                StoryHoldDetector { holding in
                    if holding {
                        ...
                        isPaused = true
                        pauseVideo()
                    } else if !scenePaused, isPaused || isHolding {
                        ...
                        //
                        // ⚠️ `!scenePaused` FOR THE SAME REASON THE `onChange` ABOVE HAS IT: leaving
                        // the app sets `isPaused` too, and this release cannot tell the two apart on
                        // its own. Without the guard a stray touch during that window would clear a
                        // pause this gesture never made and start the story playing behind the app.
                        // The scene's own return is what clears the scene's pause.
                        isPaused = false
                        isHolding = false
                        playVideo()
                    }
