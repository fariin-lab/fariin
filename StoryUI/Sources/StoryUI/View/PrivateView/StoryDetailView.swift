...
struct StoryDetailView: View {
    ...
    var body: some View {
        ...
        .onReceive(NotificationCenter.default.publisher(for: .init("jumpToStoryItem"))) { note in
            ...
            // ⚠️ THE WINDOW IS PUBLISHED FROM HERE AGAIN, and only because the row that took the job
            // over has gone back to its 552 shape (owner's order 2026-08-13, the swipe restore). That
            // row does not lay story items out, so it has no `validIds` pass to publish from, and with
            // nothing setting the window `releaseOutsideWindow` never runs and parked item views are
            // never torn down. His call: keep 565's video hygiene rather than inherit 552's leak.
            //
            // The row settled on a new card, so the preview window moves with it. This is the moment
...
private extension StoryDetailView {
    ...
    private func publishPreviewWindow(around index: Int) {
        ...
        // `Set`, because the store's window took that type in 146e0212 and the engine keeps its 565
        // shape here — this is the only line of the old publisher that had to change.
        StoryItemViewStore.setWindow(Set(model.stories[lo...hi].map(\.id)))
    }
